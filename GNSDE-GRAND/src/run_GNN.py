...
def accuracy_with_entropy_threshold(prediction_probabilities, labels, entropy_threshold, mask_data):
...
def evaluate_with_entropy_threshold(model, data, feat, entropy_threshold=np.inf, pos_encoding=None):
    model.eval()
    with torch.no_grad():
      model.odeblock.odefunc.nfe = 0
      prediction = model(feat, pos_encoding) 
      output_probability = F.softmax(prediction, dim=-1)
      accs = []
      for _, mask in data('train_mask', 'val_mask', 'test_mask'):
        acc, _ = accuracy_with_entropy_threshold(output_probability, data.y, entropy_threshold=entropy_threshold, mask_data=mask)
        accs.append(acc)
    return accs
""" End: Evaluate with Entropy """

@torch.no_grad()
def test(model, data, pos_encoding=None, opt=None, entropy_threshold=np.inf):  # opt required for runtime polymorphism
  model.eval()
  feat = data.x
  if model.opt['use_labels']:
    feat = add_labels(feat, data.y, data.train_mask, model.num_classes, model.device)
    
  accs = evaluate_with_entropy_threshold(model, data, feat, entropy_threshold=entropy_threshold, pos_encoding=pos_encoding)
  return accs
...
def main(cmd_opt):
  ...
  this_test = test if opt['dataset'] == 'ogbn-arxiv' else test
  this_train = train_OGB if opt['dataset'] == 'ogbn-arxiv' else train
  best_model = None
  ...
  for entropy_threshold in entropy_thresholds:
    _, _, test_acc = this_test(best_model, data, pos_encoding, opt, entropy_threshold)
...
if __name__ == '__main__':
  ...
  if opt['planetoid_split'] or opt['use_80_10_10_split']:
    model_name = 'GCN_planetoid_split'
  ...
  save_results(all_accuracies, model_name, entropy_results_path)