...
def main(cmd_opt):
  ...
  def initialize_graph_neural_sde(device, edge_index, num_features, hidden_dimension, num_classes):
    ...
    gnn_f = nn.Sequential(
    MyGCN(edge_index=edge_index, in_feats=hidden_dimension + 1, out_feats=hidden_dimension, activation=nn.Softplus(), dropout=0.2),
    ...
    nn.Flatten(),
    MyGCN(edge_index=edge_index, in_feats=hidden_dimension, out_feats=num_classes, activation=None, dropout=0.).to(device)
    ).to(device)
        
    # Initialize the Graph Neural SDE model
    graph_neural_sde = LatentGraphSDE(drift_net=gnn_f, in_net=gcn_in, out_net=gcn_out, sde_output_dim=hidden_dimension, device=device, opt=opt, sigma=1.0).to(device)

    return graph_neural_sde
  
  model = initialize_graph_neural_sde(device, edge_index, num_features, hidden_dim, num_classes)
...
if __name__ == '__main__':
 ...
 # new
  ...
  model_sde_name = 'TRUE_LGNSDE_sigma{}_rtol{}_t{}'.format(sigma_name, rtol_name, sde_t_name)
