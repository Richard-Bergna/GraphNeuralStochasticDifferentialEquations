...
class BaseGNSDE(torchsde.SDEIto, MessagePassing):
  def __init__(self, opt, dataset, device=torch.device('cpu'), theta=1.0, mu=0.0, sigma=5.0, adaptive=False, method="srk", rtol =0.001, atol = 0.001, t0=0, t1=0.01):
    torchsde.SDEIto.__init__(self, noise_type="diagonal")
    ...
    self.gnsde_drift_net = nn.Sequential(
      DriftLayer(in_feats=opt['hidden_dim'], out_feats=opt['hidden_dim'], activation=nn.Softplus(), dropout=self.opt['dropout']),
