...
def train(model, optimizer, data, pos_encoding=None):
  ...
  feat = data.x
  if opt['use_labels']:
    train_label_idx, train_pred_idx = get_label_masks(data, opt['label_rate'])
...
def evaluate_stochastic_model_OGB(model, data, feat, n_samples=1, entropy_threshold=np.inf, pos_encoding=None):
    model.eval()
    with torch.no_grad():
      _, samples_mean, samples_entropy = mc_sample_stats(model, feat, n_samples=n_samples, pos_encoding=pos_encoding)
      # acc, num_of_predictions = accuracy_with_entropy_threshold(samples_mean, samples_entropy, Y, entropy_threshold=entropy_threshold, mask_data=mask_data)
...
# we wrap all of this Neural ODE initializer   def initialize_graph_neural_sde(device, edge_index, num_features, hidden_dimension, num_classes):

  def initialize_graph_neural_ode(device, edge_index, num_features, hidden_dimension, num_classes):
      # Initialize GCN layers
      gcn_in = MyGCN(edge_index=edge_index, in_feats=num_features, out_feats=hidden_dimension, activation=F.relu, dropout=0.4).to(device)
      
      # Define and initialize ODE function
      
      gde_func = nn.Sequential(
          MyGCN(edge_index=edge_index, in_feats=hidden_dimension, out_feats=hidden_dimension, activation=nn.Softplus(), dropout=0.2),
          MyGCN(edge_index=edge_index, in_feats=hidden_dimension, out_feats=hidden_dimension, activation=None, dropout=0.2)
      ).to(device)
      ...
      nn.Flatten(),
      MyGCN(edge_index=edge_index, in_feats=hidden_dimension, out_feats=num_classes, activation=nn.Softmax(), dropout=0.).to(device)
      ).to(device)
  ...
  model = initialize_graph_neural_ode(device, edge_index, num_features, hidden_dim, num_classes)
  ...
  opt['optimizer'] = "adam" #  lr=opt['lr'], weight_decay=opt['decay']
  optimizer = get_optimizer(opt['optimizer'], parameters, lr=0.001)
  ...
  for entropy_threshold in entropy_thresholds:
    ...
    _, _, test_acc = this_test(best_model, data, pos_encoding, opt, entropy_threshold, n_samples=50)
    all_accuracies[str(entropy_threshold)].append(test_acc)
    print(f'Entropy Threshold: {entropy_threshold}', 'Test accuracy:', test_acc)                
...
if __name__ == '__main__':
  ...
  parser.add_argument('--epoch', type=int, default=250, help='Number of training epochs per iteration.')
  parser.add_argument('--alpha', type=float, default=1.0, help='Factor in front matrix A.')
  parser.add_argument('--alpha_dim', type=str, default='sc', help='choose either sc alar (sc) or vector (vc) alpha')
 ...
 # new
  ...
  parser.add_argument('--sigma', type=float, default=1.0, help="Diffussion constant for the SDE")
  parser.add_argument('--rtol', type=float, default=0.01, help="rtol constant for the SDE solver")
  parser.add_argument('--sde_t', type=float, default=1.0, help="rtol constant for the SDE solver")
  parser.add_argument('--use_80_10_10_split', action='store_true', help='Uses a training-val-test split of 80, 10, 10, respectively.')
  ...
  sigma_name = str(opt['sigma']) 
  rtol_name = str(opt['rtol'])
  sde_t_name = str(opt['sde_t'])
  
  model_sde_name = 'GNODE_sigma{}_rtol{}_t{}'.format(sigma_name, rtol_name, sde_t_name)
  
  if opt['planetoid_split'] or opt['use_80_10_10_split']:
    model_sde_name = 'LGNSDE_planetoid_split'
